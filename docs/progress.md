...
| 2026-04-20 | Cleanup/quality pass on all pending changes: formatted dirty PHP files with Pint, removed duplicate PH address selector script block, fixed OTP challenge cache-store behavior for local runtime (array cache fallback to file, testing keeps default store), and aligned OTP success flow token/session regeneration to avoid post-login page-expired behavior.                              | Copilot    |
| 2026-04-20 | Stabilized pending schema/test behavior by removing duplicate `application_status_submitted_idx` creation from the performance-index migration and updating apply-flow feature payloads to include required applicant `baranggay`; full test suite + build re-verified green.                                                                                                                 | Copilot    |
